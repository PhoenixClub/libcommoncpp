...
class COMMONCPP_API InetAddress
{
   ...
   * byte order.
   */
  explicit InetAddress(uint32_t addr) throw();

  /** Construct a new, InetAddress for the given IP address.
   *
   * @param addr The IP address, represented as a 4-byte tuple.
   */
  explicit InetAddress(byte_t addr[4]) throw();
   ...
   */
  inline uint32_t getAddress() const throw()
  { return(_addr); }
  ...
  /** Inequality operator. */
  inline bool operator!=(const InetAddress &other) const throw()
