...
class COMMONCPP_API ExitStatus
{
   ...
   */
  inline int getExitSignal() const throw()
  { return(_exitSignal); }
   ...
   * always 0 on Linux).
   */
  inline uint_t getMaxRSS() const throw()
  { return(_maxSize); }
 ...
 * communicate with the program via its standard I/O streams, terminate
 * the program, and collect its exit status. The class also includes a
 * number of static utility functions that affect the current
 * process.
 *
 * When the Process is created, its standard input, output, and
 * error streams are automatically connected to the parent process;
 * these streams can be accessed using the getInputStream(),
 * getOutputStream(), and getErrorStream() methods. Any output that
 * the process produces on its standard output and error streams
 * must be read in a timely manner by the calling process to avoid
...
class COMMONCPP_API Process
{
  ...
  /** Forcibly kill the process. If the process has not been
  ...
  /** Wait for the process to terminate. Once a process has exited
   * (for whatever reason) and its exit status has been collected
   * via a call to this method, it can be started again with
   * start().
   *
  ...
  inline static const char *getProgramName() throw()
  ...
  inline static void setProgramName(const char *name) throw()
  { _progname = name; }
