...
template<typename T> RefSet<T>::~RefSet() throw()
{
}
...
{
  bool added = false;
  typename std::map<T, int>::iterator iter = find(value);

  if(iter != std::map<T, int>::end())
    ((*iter).second)++;
  else
  {
    insert(std::map<T, int>::begin(), std::make_pair(value, 1));
    added = true;
  }
...
{
  typename std::map<T, int>::const_iterator iter = find(value);
