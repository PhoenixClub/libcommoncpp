...
class COMMONCPP_API ByteOrder
{
  ...
  inline static uint16_t hostToNetwork(uint16_t n) throw()
  { return(htons(n)); }

  /** Convert an unsigned 32-bit integer from host to network byte-order. */
  inline static uint32_t hostToNetwork(uint32_t n) throw()
  { return(htonl(n)); }
  ...
  inline static uint16_t networkToHost(uint16_t n) throw()
  { return(ntohs(n)); }

  /** Convert an unsigned 32-bit integer from network to host byte-order. */
  inline static uint32_t networkToHost(uint32_t n) throw()
  { return(ntohl(n)); }
   ...
   * otherwise.
   */
  inline static bool isLittleEndian() throw()
  { return(!isBigEndian()); }
   ...
   */
  inline static Endianness getEndianness() throw()
   ...
   * @return The value.
   */
  template<typename T> static T & reverseBytes(T &value) throw()
  {
  ...
  static void _reverseBytes(byte_t *buf, size_t length) throw();
