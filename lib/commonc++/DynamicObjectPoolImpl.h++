...
template <class T>
  DynamicObjectPool<T>::DynamicObjectPool(size_t size,
                                          void (*resetFunc)(T *),
                                          bool lazy /* = false */)
    : ObjectPool<T>(size),
...
template <class T>
  DynamicObjectPool<T>::~DynamicObjectPool() throw()
{
  Link *p = _list;

  for(size_t i = 0; i < ObjectPool<T>::_size; ++i)
  {
    delete p->data;
    ++p;
  }
...
template <class T>
  T * DynamicObjectPool<T>::reserve() throw(ObjectPoolException)
{
  if(_freeList == NULL)
...
template <class T>
  void DynamicObjectPool<T>::release(T *elem) throw(ObjectPoolException)
{
  if(!elem || !_usedList)
    throw ObjectPoolException();
