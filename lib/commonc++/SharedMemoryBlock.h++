...
/** An encapsulation of a block of memory that can be mapped by one
 * or more (not necessarily related) processes; when one process
 * changes the data in the block, the changes are immediately
 * visible to the other processes.
 *
 * @author Mark Lindner
 */
